...
*/
{% macro TJCOverallEncounter_with_Comfort_Measures_during_Hospitalization(state) %}{{ _operators.printOperator(state, { 'operator': _operators.ExpressionDef, 'context': 'Patient', 'name': 'Encounter with Comfort Measures during Hospitalization', 'child': { 'operator': _operators.Query, 'where': none, 'returnClause': none, 'sortClause': none, 'children': [{ 'operator': _operators.AliasedQuerySource, 'alias': 'IschemicStrokeEncounter', 'child': { 'operator': _operators.ExpressionRef, 'reference': TJCOverallIschemic_Stroke_Encounter } }, { 'operator': _operators.With, 'alias': 'ComfortMeasure', 'child': { 'operator': _operators.ExpressionRef, 'reference': TJCOverallIntervention_Comfort_Measures }, 'suchThat': { 'operator': _operators.InInterval, 'left': { 'operator': _operators.Coalesce, 'children': [{ 'operator': _operators.Start, 'child': { 'operator': _operators.FunctionRef, 'reference': Global.MATGlobalCommonFunctionsNormalizeInterval, 'children': [{ 'operator': _operators.Property, 'scope': 'ComfortMeasure', 'path': 'relevantDatetime', 'child': none }, { 'operator': _operators.Property, 'scope': 'ComfortMeasure', 'path': 'relevantPeriod', 'child': none }] } }, { 'operator': _operators.Property, 'scope': 'ComfortMeasure', 'path': 'authorDatetime', 'child': none }] }, 'right': { 'operator': _operators.FunctionRef, 'reference': Global.MATGlobalCommonFunctionsHospitalizationWithObservation, 'children': [{ 'operator': _operators.AliasRef, 'name': 'IschemicStrokeEncounter' }] } } }], 'letClauseList': [] } }) }}{% endmacro %}
...
*/
{% macro TJCOverallCalendarDayOfOrDayAfter(state) %}{{ _operators.printOperator(state, { 'operator': _operators.FunctionDef, 'context': 'Patient', 'name': 'CalendarDayOfOrDayAfter', 'child': { 'operator': _operators.Interval, 'high': { 'operator': _operators.FunctionRef, 'reference': TJCOverallTruncateTime, 'children': [{ 'operator': _operators.Add, 'left': { 'operator': _operators.OperandRef, 'reference': 'StartValue' }, 'right': { 'operator': _operators.Quantity, 'unit': 'days', 'value': '2' } }] }, 'highClosedExpression': none, 'low': { 'operator': _operators.FunctionRef, 'reference': TJCOverallTruncateTime, 'children': [{ 'operator': _operators.OperandRef, 'reference': 'StartValue' }] }, 'lowClosedExpression': none }, 'typeSpecifier': { 'operator': _operators.IntervalTypeSpecifier, 'children': [{ 'operator': _operators.NamedTypeSpecifier, 'name': '{urn:hl7-org:elm-types:r1}DateTime', 'children': [] }] }, 'operators': [{ 'operator': _operators.OperandDef, 'name': 'StartValue', 'children': [{ 'operator': _operators.NamedTypeSpecifier, 'name': '{urn:hl7-org:elm-types:r1}DateTime', 'children': [] }] }] }) }}{% endmacro %}
