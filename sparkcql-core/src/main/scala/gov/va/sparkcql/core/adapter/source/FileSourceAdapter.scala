...
class FileSourceAdapter(val modelAdapters: CompositeModelAdapter, val spark: SparkSession, path: String) extends SourceAdapter {
  ...
  def acquireData(dataType: QName): Option[Dataset[Row]] = {
    ...
    if (jsonData.length > 0) {
      val schema = modelAdapters.schemaOf(dataType)
      if (schema.isDefined) {
